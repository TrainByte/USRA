...
def main(args):
    ...
    save_model_path = os.path.join(model_dir,
                             f"{json_file_trained}_B{args.B / 1e6}_Pmax{args.P_max}_Cpu{args.cpu_cycles / 1e9}_Tp{args.T_p}_z_n{args.z_n}_NOMA{args.NOMA}_drop{args.drop}_beta{format(args.beta,'g')}_hiddenWidth{args.hidden_width}_lrA{args.lr_a}_lrC{args.lr_c}_gamma{args.gamma}")
    agent_PPO.load_model(save_model_path)
    ...
    # mean=agent_PPO.state_norm_mean,
    # std=agent_PPO.state_norm_std
    # )
    ...
    for t in range(total_samples):  # 对于整个total_samples时间轴上的数据分帧进行处理
        ...
        if (t > 0):
            for agent in range (u):
                ...
                a_time = time.time()
                strategy = agent_PPO.evaluate(current_state)
                
                all_channel_times.append(time.time()-a_time)  
        ...
        all_compute_times.append(time.time()-a_time)
        '''
        ...
        sum_utility_current, p_current, power_no_sloved,power_time = pa_cvx.power_allocation_cvx(max_iter,
                                                                                        ...
                                                                                        NOMA,
                                                                                        dropped,weighted)
        all_power_times.append(power_time)
