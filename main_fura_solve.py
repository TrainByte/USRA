...
def main(args):
    ...
    for t in range(total_samples):  # 对于整个total_samples时间轴上的数据分帧进行处理
        ...
        while low <= high:
            ...
            # 根据用户到达顺序选择前 selection_users_num 个用户
            # 排序索引按照用户到达顺序
            sorted_indices = np.argsort(user_order_matrix[:,t])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", default="fura_output", type=str, help="Output directory.")
    # 新添加的参数
