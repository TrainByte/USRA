...
def main(args):
    ...
    queue_number = args.queue_number  #队列的数量
    T_p = args.T_p
    NIND = args.NIND
    NOMA = args.NOMA
    dropped = args.drop
    ...
    T_m_k_only_c_matrix = np.zeros(cpu_matrix.shape)     # 纯计算时间
    T_m_k_queue_matrix = np.zeros(cpu_matrix.shape)    # 排队时间
    ...
    for t in range(total_samples):  # 对于整个total_samples时间轴上的数据分帧进行处理
        print(f"##########################第{t}帧#################################")
        print(
            f"{json_file}_B{args.B / 1e6}_Pmax{args.P_max}_Cpu{args.cpu_cycles / 1e9}_Tp{args.T_p}_z_n{args.z_n}_NIND{args.NIND}_NOMA{args.NOMA}_drop{args.drop}_beta{format(args.beta,'g')}_queue{queue_number}.npz")
        num_iter = 0
        ...
        # 基于初始化的功率和信道分配计算上传延迟
        latency_inital,_ = pb.user_uplink_time(H_all_2[t], channel_initial, p_initial, task_matrix[:, t], u, W,
                                                      m, cell_mapping[:, t], T_p, NOMA,
                                                      dropped,weighted)
        # 基于FIFO和queue_number规则，并根据上传延迟计算排队延迟和计算延迟
        computing_latency_inital, queue_latency_inital = pb.queue_computing_delay(m, cpu_matrix[:, t], task_matrix[:, t],
                                                                        cpu_cycles, cell_mapping[:, t],
                                                                        channel_initial,latency_inital.flatten(),queue_number)
        t_m_k_c_initial = queue_latency_inital + computing_latency_inital  # 总MEC的时延
        ...
        while num_iter < max_iter:
            ...
            if num_iter == 0:
                ...
                problem = MyProblem_Drop(H_all_2[t], p_previous, u, n, m, cpu_cycles, cell_mapping[:, t],
                                         ...
                                         T_p, cpu_matrix[:, t], task_matrix[:, t], W, NOMA, dropped, z_n, weighted,queue_number=queue_number)
                ...
                start_time = time.time()
                # 基于FIFO和queue_number规则，并根据上传延迟计算排队延迟和计算延迟
                computing_latency_current, queue_latency_current = pb.queue_computing_delay(m, cpu_matrix[:, t], task_matrix[:, t],
                                                                                cpu_cycles, cell_mapping[:, t],
                                                                                channel_initial,latency_current.flatten(),queue_number)
                t_m_k_c_current = queue_latency_current + computing_latency_current  # 总MEC的时延
                ...
                if sum_utility_current > sum_utility_previous:
                    ...
                    sum_utility_previous, channel_previous, t_m_k_c_previous = sum_utility_current, channel_current, t_m_k_c_current
                else:  # 无解
                    print('子信道无更好解:', sum_utility_current)
                    sum_utility_list.append(sum_utility_previous)
                    sum_utility_current, channel_current, t_m_k_c_current = sum_utility_previous, channel_previous, t_m_k_c_previous
                '''
                power allocation
                '''
        ...
        # 基于FIFO和queue_number规则，并根据上传延迟计算排队延迟和计算延迟
        computing_latency_opt, queue_latency_opt = pb.queue_computing_delay(m, cpu_matrix[:, t], task_matrix[:, t],
                                                                        cpu_cycles, cell_mapping[:, t],
                                                                        channel_opt,T_m_k_uplink.flatten(),queue_number)
        T_m_k_only_c_matrix[:, t] = computing_latency_opt  # 纯计算的时延
        T_m_k_queue_matrix[:, t] = queue_latency_opt # 排队的时延
        T_m_k_c_matrix[:, t] = queue_latency_opt + computing_latency_opt  # 总MEC的时延
    ...
    np.savez(save_path,
             ...
             T_m_k_only_c_matrix=T_m_k_only_c_matrix,
             T_m_k_queue_matrix=T_m_k_queue_matrix,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--queue_number", type=int, default=3, help="MEC queue number.")
