...
class userselection_subchannelscheduling:
    def __init__(self, options,options_policy,u,m,n,noise_var,seed=None):
        ...
        self.policynum_input = (7+n)*u
        self.policynum_actions = (n+1)**u # Kumber of actions  每个用户有N+1个选择
    
        self.previous_state = np.zeros(self.policynum_input) * 1
        self.previous_action = np.ones(self.N) * 1
        self.previous_action_logprob = np.ones(self.N) * 1
        self.previous_reward = np.ones(1)

       
        
    def local_state(self, H_all_2,cpu_matrix, task_matrix, cell_mapping,T_requirement_matrix,weighted,US_list,reward_list,status_matrix_list):
        ...
        global_state = self.state_concatenate(H_all_2,cpu_matrix, task_matrix, cell_mapping,T_requirement_matrix,weighted,US_list,reward_list,status_matrix_list)
        return global_state
    
    def state_concatenate(self,H_all_2,cpu_matrix, task_matrix, cell_mapping,T_requirement_matrix,weighted,US_list,reward_list,status_matrix_list):
        state = np.zeros(self.policynum_input)
        cursor = 0
        for i in range(self.U):
            state[cursor:cursor+self.N] = np.log10(H_all_2[i,i,:]/self.scale_gain)
            cursor += self.N

            state[cursor] = cpu_matrix[i] / 1000
            cursor += 1

            state[cursor] = task_matrix[i] / 10000
            cursor += 1

            state[cursor] = T_requirement_matrix[i] * 1000
            cursor += 1

            state[cursor] = weighted[i]
            cursor += 1
            
            state[cursor] = US_list[i]
            cursor += 1

            state[cursor] = reward_list[i]
            cursor += 1

            state[cursor] = status_matrix_list[i]
            cursor += 1
