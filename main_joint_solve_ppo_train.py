...
def main(args):
    ...
    # epochs = args.epochs
    # eps = args.eps
    # MIN_BATCH_SIZE = args.minibatch_size
    
    # DRL 初始化内容
    policy = US_One_agent_env.userselection_subchannelscheduling(options,options_policy,u,m,n,noise_var,seed=100)
    args.state_dim = policy.policynum_input
    args.action_dim = policy.policynum_actions
    replay_buffer = ReplayBuffer(args)
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    agent_PPO = PPO.PPO_discrete(args,device)
    US_strategy = np.array(u)
    ...
    for t in range(total_samples):  # 对于整个total_samples时间轴上的数据分帧进行处理
        ...
        if (t % train_episodes['T_train'] > 1):

            current_state = policy.local_state(H_all_2[t],cpu_matrix[:, t], task_matrix[:, t], cell_mapping[:, t],T_requirement_matrix[:, t],weighted,US_list[-1],rewards_list[-1],status_matrix_list[-1])
            current_state = state_norm(current_state)
            a_time = time.time()
            strategy, strategy_logprob = agent_PPO.choose_action(current_state)
            # print(strategy)
            time_calculating_strategy_takes.append(time.time()-a_time)

            if (t % train_episodes['T_train'] > 2):
                current_reward = sum_utility_list[-1]

                # if forcezero: 
                #     current_reward -= sum_utility_list[-1]
                #     current_reward -= -5
                
                replay_buffer.store(policy.previous_state, policy.previous_action, policy.previous_action_logprob, current_reward, current_state, False, False)
                # # current_reward = reward_scaling(current_reward)  # Trick 4
                # transition_dict['states'].append(policy.previous_state)
                # transition_dict['actions'].append(policy.previous_action)
                # transition_dict['next_states'].append(current_state)
                # transition_dict['rewards'].append(current_reward)
                # transition_dict['dones'].append(False)
            

                # if len(transition_dict['states']) >= args.batch_size:  # Replace MIN_BATCH_SIZE with your mini-batch size
                if replay_buffer.count == args.batch_size:
                    a_time = time.time()
                    # agent_PPO.update(transition_dict, t%train_episodes['T_train'])
                    agent_PPO.update(replay_buffer,total_samples)
                    replay_buffer.count = 0
                    time_optimization_at_each_slot_takes.append(time.time()-a_time)
                    # transition_dict= {'states': [], 'actions': [], 'next_states': [],
                    #                                 'rewards': [], 'dones': []}
            
            quotient = strategy.copy()   
            # Pick the action
            for i in range (u):
                US_strategy[i] = quotient%(n+1)
                quotient = quotient // (n+1)


            
            # Add current state to the short term memory to observe it during the next state
            policy.previous_state = current_state
            policy.previous_action = strategy
            policy.previous_action_logprob = strategy_logprob
        ...
        sum_utility_list.append(np.mean(Utility_m_k_matrix_t)*1000)
        ...
        if (t+1) % 1000 == 0:
            # 创建 train/model 目录，如果不存在的话
            model_dir = os.path.join(args.output_dir, 'train', 'model')
            if not os.path.exists(model_dir):
                os.makedirs(model_dir, exist_ok=True)
            # Training loop结束后
            agent_PPO.state_norm_mean = state_norm.running_ms.mean
            agent_PPO.state_norm_std = state_norm.running_ms.std
            checkpoint = {'actor_state_dict': agent_PPO.actor.state_dict(),
                        'critic_state_dict': agent_PPO.critic.state_dict(),
                        'optimizer_actor_state_dict': agent_PPO.optimizer_actor.state_dict(),
                        'optimizer_critic_state_dict': agent_PPO.optimizer_critic.state_dict(),
                        'K_epochs': agent_PPO.K_epochs,
                        'epsilon': agent_PPO.epsilon,
                        'gamma': agent_PPO.gamma,
                        'max_train_steps': agent_PPO.max_train_steps,
                        'state_norm_mean': agent_PPO.state_norm_mean,
                        'state_norm_std': agent_PPO.state_norm_std
                        }
            save_model_path = os.path.join(model_dir,
                             f"{json_file}_B{args.B / 1e6}_Pmax{args.P_max}_Cpu{args.cpu_cycles / 1e9}_Tp{args.T_p}_z_n{args.z_n}_NOMA{args.NOMA}_drop{args.drop}_beta{format(args.beta,'g')}_hiddenWidth{args.hidden_width}_lrA{args.lr_a}_lrC{args.lr_c}_gamma{args.gamma}")
            torch.save(checkpoint, save_model_path)
            print("Model saved in path: %s" % save_model_path)   
    
    # 创建 train/data 目录，如果不存在的话
    data_dir = os.path.join(args.output_dir, 'train', 'data')
    if not os.path.exists(data_dir):
        os.makedirs(data_dir, exist_ok=True)   
    save_data_path = os.path.join(data_dir,
                             f"{json_file}_B{args.B / 1e6}_Pmax{args.P_max}_Cpu{args.cpu_cycles / 1e9}_Tp{args.T_p}_z_n{args.z_n}_NOMA{args.NOMA}_drop{args.drop}_beta{format(args.beta,'g')}_hiddenWidth{args.hidden_width}_lrA{args.lr_a}_lrC{args.lr_c}_gamma{args.gamma}.npz")
    # os.makedirs(data_dir, exist_ok=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--json-file', type=str,
                        default='train_M2_U6_N2_Task_u15000_Ucpu_e1000_Tdemand_u15',
    ...
    parser.add_argument("--output_dir", default="ppo_output", type=str, help="Output directory.")
    # 新添加的参数
