...
import argparse
import json
from SEGA_NoKKT_Drop import MyProblem_Drop_NoKKT
...
def main(args):
    ...
    for t in range(total_samples):  # 对于整个total_samples时间轴上的数据分帧进行处理
        ...
        x_initial = np.ones(u)  # 初始化为1,经过compute_computing_delay会变成均匀分配
        t_m_k_c_initial, x_initial_tmep = pb.compute_computing_delay(m, cpu_matrix[:, t], task_matrix[:, t],
                                                                            cpu_cycles, cell_mapping[:, t],
                                                                            channel_initial,x_initial)  # 初始化为全调度最优分配
        ...
        joint communication and computing allocation:including computing_resource、power and sub-channel
        '''
        # 本帧内每一轮CEAO记录E2GA的代数与nfev（与channel_time_list等对应）
        e2ga_gens_list = []
        while num_iter < max_iter:
            ...
            if num_iter == 0:
                ...
                channel allocation
                '''
                problem = MyProblem_Drop_NoKKT(H_all_2[t], p_previous, u, n, m, cpu_cycles, cell_mapping[:, t],
                ...
                '''
                start_time = time.time()
                t_m_k_c_current, x_current = pb.compute_computing_delay(m, cpu_matrix[:, t],
                                                                                    task_matrix[:, t],
                                                                                    cpu_cycles, cell_mapping[:, t],
                                                                                    channel_current,x_current)  # 根据当前信道分配情况计算时延
                end_time = time.time()
        ...
        T_m_k_c_matrix[:, t], _ = pb.compute_computing_delay(m, cpu_matrix[:, t], task_matrix[:, t],
                                                                         cpu_cycles, cell_mapping[:, t],
                                                                         channel_opt,x_opt)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", default="ceao_NoKKT_output", type=str, help="Output directory.")
    # 新添加的参数
