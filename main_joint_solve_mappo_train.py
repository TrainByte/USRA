...
def main(args):
    ...
    print(f"修改前的 batch_size: {args.batch_size}")
    args.batch_size = int((args.batch_size)*u/42)  # 因为基准是42个用户对应的,即51轮进行一次训练,换了u也要换buffer size
    print(f"修改后的 batch_size: {args.batch_size}")
    ...
    for t in range(total_samples):  # 对于整个total_samples时间轴上的数据分帧进行处理
        ...
        print(f"##########################第{t}帧#################################")
        print(
            f"{json_file}_B{args.B / 1e6}_Pmax{args.P_max}_Cpu{args.cpu_cycles / 1e9}_Tp{args.T_p}_z_n{args.z_n}_NOMA{args.NOMA}_drop{args.drop}_beta{format(args.beta,'g')}")
        ...
        if (t % train_episodes['T_train'] > 1):
            for agent in range (u):
                ...
                if (t % train_episodes['T_train'] > 2):
                    current_reward = sum_utility_list[-1][agent]

                    if forcezero: 
                        current_reward -= sum_utility_list[-1][agent]
                        current_reward -= -5
                    
                    replay_buffer.store(policy.previous_state[agent,:], policy.previous_action[agent], policy.previous_action_logprob[agent], current_reward, current_state, False, False)
                ...
                policy.previous_action_logprob[agent] = strategy_logprob
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", default="mappo_output", type=str, help="Output directory.")
    # 新添加的参数
