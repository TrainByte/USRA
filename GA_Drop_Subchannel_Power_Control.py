...
class MyProblem_Drop(ea.Problem):  # 继承geatpy中的Problem父类
    ...
    def __init__(self, H_data, Pmax, u, n, m,cpu_cycles, cell_mapping, T_requirement, T_p, Cpu_matrix, Task_matrix, W, NOMA, dropped,
                 z_n, weighted):
        name = 'z'
        self.U = u
        self.N = n
        ...
        Dim = self.U * 2  # 决策变量维数
        dim_one_var = self.U
        varTypes = [1] * dim_one_var + [0] * dim_one_var  # 决策变量的类型列表，0：实数；1：整数
        if self.dropped:
            lb_subchannel = [0] * dim_one_var  # 决策变量下界
        else:
            lb_subchannel = [1] * dim_one_var  # 未开启drop时所选信道号至少为1
        lb_power = [0] * dim_one_var
        lb = lb_subchannel + lb_power
        ub = [self.N] * dim_one_var + [self.Pmax] * dim_one_var  # 决策变量上界
        lbin = [1] * dim_one_var + [0] * dim_one_var  # 决策变量下边界（0表示不包含该变量的下边界，1表示包含）
    ...
    def evalVars(self, Vars):  # 定义目标函数（含约束）
        ...
        for k in range(Vars_rows):
            vars_temp = (Vars[k, :self.U]).astype(int)  # 第k个种群的决策变量，为U*1的矩阵，里面存储的为U个用户决策的信道号
            p_temp = Vars[k, self.U:].reshape(-1, 1)
            ...
            if self.NOMA is True:
                A_temp = pb.NOMA_handle(vars_temp, OMA_temp, self.m, self.cell_mapping, H_gain_data, p_temp)
            else:
                A_temp = OMA_temp

            y_star = np.sqrt(np.diag(H_gain_data).reshape(-1, 1) * p_temp) / (
                    H_gain_data * A_temp @ p_temp + 1)  # 公式（33）

            # 识别被丢弃的用户（信道号为0）
            discarded_users = vars_temp.reshape(-1, 1) == 0
            # 计算所有用户违反约束的情况值
            all_users_cv_value = (1.0 / np.log2(1 + 2 * np.multiply(y_star, np.sqrt(
                np.multiply(np.diag(H_gain_data).reshape(-1, 1), p_temp))) - np.multiply(np.square(y_star), (
                    H_gain_data * A_temp @ p_temp + 1)))) - t_constraint  # 所有U个用户违反的情况值
            
            all_users_com_delay = (self.task_matrix.reshape(-1, 1) / (self.W * np.log2(1 + 2 * np.multiply(y_star, np.sqrt(
                np.multiply(np.diag(H_gain_data).reshape(-1, 1), p_temp))) - np.multiply(np.square(y_star), (
                    H_gain_data * A_temp @ p_temp + 1)))))  # 所有U个用户的上传时延
            ...
            #                                     np.sqrt(np.multiply(np.diag(H_gain_data).reshape(-1, 1), p_temp)))
            #                 - np.multiply(np.square(y_star), (H_gain_data * A_temp @ p_temp + 1))
            #             )))
            #         )
            #     )
            # )
