...
def main(args):
    ...
    agent_DQN = DQNWithAttention(args.state_dim,hidden_dim,args.action_dim,lr,gamma,epsilon_drl,target_update,device)
    ...
    for t in range(total_samples):  # 对于整个total_samples时间轴上的数据分帧进行处理
        ...
        if (t % train_episodes['T_train'] > 1):
            for agent in range (u):
                ...
                a_time = time.time()
                strategy = agent_DQN.take_action(current_state)
                # print(strategy)
                ...
                if (t % train_episodes['T_train'] > 2):
                    ...
                    # is_terminal = (t % train_episodes['T_train'] == train_episodes['T_train'] - 1)
                    replay_buffer.add(policy.previous_state[agent,:], policy.previous_action[agent], current_reward, current_state, False)
                    ...
                    if agent == (u-1):
                        ...
                        if replay_buffer.size() > minimal_size:
                            ...
                            agent_DQN.update(transition_dict,total_samples)
        ...
        if len(np.unique(US_strategy_current)) == 1 :
            print('t %d all same'%(t))
            forcezero = True
        ...
        if (t+1) % 1000 == 0:
            ...
            # Training loop结束后
            agent_DQN.state_norm_mean = state_norm.running_ms.mean
            agent_DQN.state_norm_std = state_norm.running_ms.std
            checkpoint = {'q_net_state_dict': agent_DQN.q_net.state_dict(),
                        'target_q_net_state_dict': agent_DQN.target_q_net.state_dict(),
                        'optimizer_state_dict': agent_DQN.optimizer.state_dict(),
                        'gamma': agent_DQN.gamma,
                        'state_norm_mean': agent_DQN.state_norm_mean,
                        'state_norm_std': agent_DQN.state_norm_std
                        }
            save_model_path = os.path.join(model_dir,
                             f"{json_file}_B{args.B / 1e6}_Pmax{args.P_max}_Cpu{args.cpu_cycles / 1e9}_Tp{args.T_p}_z_n{args.z_n}_NOMA{args.NOMA}_drop{args.drop}_beta{format(args.beta,'g')}_hiddenWidth{args.hidden_width}_lr{args.lr}_gamma{args.gamma}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", default="madaqn_output", type=str, help="Output directory.")
    # 新添加的参数
    ...
    parser.add_argument("--lr", type=float, default=1e-4, help="Learning rate of dqn")
