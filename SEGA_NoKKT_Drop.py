...
class MyProblem_Drop_NoKKT(ea.Problem):  # 继承geatpy中的Problem父类
    ...
    def __init__(self, H_data, P_data, u, n, m, cpu_cycles, cell_mapping, T_requirement, T_p, Cpu_matrix, Task_matrix, W, NOMA, dropped,
        ...
        lb_comput = [0]* dim_one_var  #计算资源下界
        lb = lb_subchannel + lb_comput  #合并的下界
        ub = [self.N] * dim_one_var + [1] * dim_one_var  # 决策变量上界
    ...
    def evalVars(self, Vars):  # 定义目标函数（含约束）
        ...
        for k in range(Vars_rows):
            ...
            comput_temp = Vars[k,self.U:]  # 第k个种群的计算资源分配比例变量
            
            # 根据分配信道和计算资源比例,计算计算时延
            T_m_k_c_matrix,_ = pb.compute_computing_delay(self.m, self.cpu_matrix, self.task_matrix,
                                                                            self.cpu_cycles, self.cell_mapping,
                                                                            vars_temp,comput_temp) 
